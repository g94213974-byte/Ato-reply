...
_login_sessions = {}  # {temp_key: {'phone': ..., 'client': ..., 'step': 'otp'|'2fa', ...}}
...
async def start_single_account(session_string):
    try:
        ...
        if is_restricted:
            logger.info(f"⚠️ Restricted account added but DISABLED: {me.first_name}")
            try:
...
async def check_account_restrictions():
    """Periodically check if any accounts became restricted"""
    while True:
        ...
        for i, acc in enumerate(accounts[:]):
            ...
            try:
                ...
                try:
                    ...
                    _save_sessions()
                    _save_restricted()
                    await admin_broadcast(f"⚠️ **Account Restricted!**\n\n👤 {acc['name']}\n🆔 `{acc['id']}`\n\nAccount has been removed and logged out.")
            except:
                pass
...
async def add_account(update, context, session):
    ...
    try:
        ...
        if acc and 'error' not in acc:
            ...
            await msg.edit_text(f"⚠️ **Account is RESTRICTED/Banned!**\n\nAuto-logged out and disabled.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="main_menu")]]))
        else:
            await msg.edit_text(f"❌ Failed!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="main_menu")]]))
    except Exception as e:
    ...
    if awaiting.startswith('login_phone_'):
        ...
        if result['status'] == 'success':
            ...
            if acc and 'error' not in acc:
                ...
                await update.message.reply_text("⚠️ Account may be restricted. Check accounts menu.", reply_markup=back)
            ...
            await update.message.reply_text(f"⚠️ **Account is RESTRICTED!**\n\nCannot add this account.", reply_markup=back)
        else:
            await update.message.reply_text(f"❌ {result['message']}", reply_markup=back)
        ...
        context.user_data['awaiting'] = ''
        await update.message.reply_text(f"✅ **User-Specific Reply Added!**\n\nUser: `{uid}`\nKeyword: `{kw}`\nReply ID: {rid}\n\n✅ Note: This reply will only work for this specific user!", reply_markup=back, parse_mode='Markdown')
    ...
    if awaiting == 'contact_login':
        ...
        if contact:
            ...
            if result['status'] == 'otp_sent':
                ...
                context.user_data['awaiting'] = ''
                await update.message.reply_text(f"❌ {result.get('message', 'Login failed')}")
...
async def button_callback(update, context):
    ...
    if data == "main_menu":
        ...
        if not accounts:
            await query.edit_message_text("👥 **No accounts!**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📱 Login with Phone", callback_data="menu_login")],[InlineKeyboardButton("➕ Add String", callback_data="add_account_how")],[InlineKeyboardButton("🔙", callback_data="main_menu")]]))
            return
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="menu_user_reply")]])
        )
    
    elif data == "list_user_replies":
        user_replies = get_all_user_replies()
        if not user_replies:
            await query.edit_message_text("📭 **No user-specific replies!**\n\nAdd one from the menu.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="menu_user_reply")]]))
            return
        msg = "📋 **User-Specific Replies**\n\n"
        kb = []
        for rid, uid, kw, rt, tp in user_replies[:20]:
            msg += f"ID:{rid} | User:`{uid}` | `{kw[:15]}`\n  ➜ {rt[:30]}...\n\n"
            kb.append([InlineKeyboardButton(f"🗑 ID:{rid} (User:{uid})", callback_data=f"del_user_reply_{rid}")])
        kb.append([InlineKeyboardButton("🔙", callback_data="menu_user_reply")])
        ...
        await query.edit_message_text("✅ Deleted!" if result else "❌ Not found!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="menu_user_reply")]]))
        ...
        accs = "\n".join([f"{'🟢' if a.get('enabled',True) else '⚪'} #{i+1} {a['name']} {'🤖' if a.get('mode')=='ai' else '📋'} {'🔴R' if a.get('restricted') else ''}" for i,a in enumerate(accounts)]) or "No accounts"
...
async def run_bot():
    ...
    asyncio.create_task(check_account_restrictions())  # ✅ Added
